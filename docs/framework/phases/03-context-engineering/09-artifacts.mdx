...
slug: /framework/phases/context-engineering/artifacts
...
sidebar_position: 3
tags: [phases, context-engineering, artifacts, project-context, prd]
---

# The artifacts of this phase

Each phase of the Problem-Driven AI methodology produces specific artifacts. They are not optional. They are the evidence that the work was done rigorously and the operating system that powers everything that follows. In Context Engineering, the artifacts are six:

| Artifact | Element of the tripod | When it's created | Who produces it | Who consumes it |
|---|---|---|---|---|
| **project-context.md** | Rules | First. Base of everything. Updated when the Architect Agent generates new Rules. | Context Engineer (initial), Architect Agent (updates) | All Agents, before every Skill. |
| **Agent Definitions** | Agents | Together with Rules. Identity, responsibilities, limits, Rules they apply, gap protocol. | Context Engineer | Context Engineer (design), AI (execution). |
| **PRD** | Strategic document | After initial Rules. Translated from Solution Brief. | PM Agent | Architect Agent, SM Agent, QA Agent. |
| **Architecture Document** | Strategic (generates new Rules) | After PRD. Technical translation of PRD. | Architect Agent | SM Agent, Dev Agent, QA Agent. |
| **Story Files (Skills)** | Skills | After PRD + Architecture. Self-contained tasks. | SM Agent | Dev Agent. One Skill per interaction. |
| **Decision Log** | Cross-cutting (changes to Rules, Agents, Skills) | From the first document. Continuous. | All participants | Context Engineer, QA Agent, everyone. |
...
## Artifact 1: project-context.md (Rules)
...
See the [Anatomy page](/framework/phases/context-engineering/context-system-anatomy) for the complete structure and template.

## Artifact 2: Agent Definitions
...
## Artifact 3: PRD (Product Requirements Document)
...
- **Key consumer:** The SM Agent uses the PRD to decompose the solution into Skills. The QA Agent uses the success criteria to validate outputs.

## Artifact 4: Architecture Document
...
When the Gate Review is passed, all six artifacts travel together to Phase 4 (AI Build):
