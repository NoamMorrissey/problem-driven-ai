...
slug: /framework/phases/context-phase/step-by-step-process
...
sidebar_position: 1
tags: [phases, context-phase, process, agents, rules, skills]
hide_title: true
---
...
Context Phase is not a creative writing exercise. It's a structured translation process that converts the human understanding captured in Phases 1 and 2 into a system that AI can process faithfully. Every step has a clear purpose, a concrete deliverable, and a validation criterion.

<StepAccordion>
<StepAccordionItem number={1} title="Step 1. Establish Rules (project-context.md)" summary="Create the project-context.md constitution: extract every global decision from the Solution Brief and calibrate precision levels.">
...
Not all Rules need the same rigidity. The key is to calibrate: invest precision where the impact on Agents and Skills is high, and delegate where it's low. Level 5 is absolute restriction. Level 1 is conscious delegation.

<WeightChart bars={[
  {label: 'Level 5 — Prescriptive', percent: 100, color: 'var(--ifm-color-danger)'},
  {label: 'Level 4 — Specific', percent: 80, color: 'var(--ifm-color-warning)'},
  {label: 'Level 3 — Directed', percent: 60, color: 'var(--ifm-color-primary)'},
  {label: 'Level 2 — Orientative', percent: 40, color: 'var(--ifm-color-info)'},
  {label: 'Level 1 — Delegated', percent: 20, color: 'var(--pdai-gray-300)'},
]} />
...
<StepAccordionItem number={2} title="Step 2. Design the Agent Chain" summary="Define the chain of specialized Agents that will translate the Solution Brief into a complete context system.">
...
Most projects use five Agents in sequence. Each Agent transforms the previous one's output but never contradicts it, never enriches it with invention, and never reinterprets it. Like a relay race: each runner carries the same baton.
...
- **Analyst Agent** — Translates human artifacts into structured requirements. Does not discover or invent — it reformats.
- **PM Agent** — Generates the PRD: what to build, for whom, with what priority. Defines use cases and success criteria.
- **Architect Agent** — Translates PRD into technical decisions. The only Agent that updates the project-context.md with new Rules.
- **SM Agent** — Decomposes the solution into self-contained Skills. Each Skill includes all context for its executing Agent.
- **QA Agent** — Validates fidelity, completeness, consistency with Rules. Checks at every chain link, not just the end.
...
<StepAccordionItem number={3} title="Step 3. Generate Strategic Documents" summary="Produce the PRD and Architecture Document — the intermediate layer between Rules and Skills.">
...
<StepAccordionItem number={4} title="Step 4. Decompose into Skills" summary="Break everything into self-contained Skills that development Agents will execute.">
...
<StepAccordionItem number={5} title="Step 5. Validate through Outputs" summary="Run the system, judge the results, and iterate until outputs faithfully express the Problem Statement.">
