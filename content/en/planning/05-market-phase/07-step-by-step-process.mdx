...
slug: /framework/phases/market-phase/step-by-step-process
...
sidebar_position: 1
tags: [framework, phases, market-phase, process, step-by-step]
hide_title: true
---
...
<StepAccordion>
<StepAccordionItem number={1} title="Step 1. Measurement system activation" summary="Connect the KPI & OKR Register to concrete instrumentation and activate the Signal Log.">
...
<StepAccordionItem number={2} title="Step 2. Signal capture" summary="Operate continuous capture across automatic, structured, and contextual layers.">
...
Automation degree of signals ingested into the system.

<EffortChart segments={[
  {label: 'Automatic (KPIs)', percent: 50, color: 'var(--ifm-color-success)'},
  {label: 'Structured (Feedback)', percent: 30, color: 'var(--ifm-color-danger)'},
  {label: 'Contextual (Market)', percent: 20, color: 'var(--ifm-color-primary)'},
]} />

- **Automatic layer (50%)** — KPIs connected directly to data sources. 100% automated ingestion.
- **Structured layer (30%)** — Feedback processed with AI. Sentiment analysis and automatic categorization.
- **Contextual layer (20%)** — Market signals captured by humans. Qualitative context and expert interpretation.
...
<StepAccordionItem number={3} title="Step 3. Signal interpretation" summary="Transform raw signals into knowledge through weekly, monthly, and quarterly interpretation sessions.">
...
Time dedicated to interpretation by session type.

<WeightChart bars={[
  {label: 'KPI review (Weekly)', percent: 20, color: 'var(--ifm-color-primary-lightest)'},
  {label: 'Feedback synthesis (Monthly)', percent: 55, color: 'var(--ifm-color-primary-light)'},
  {label: 'Strategic (Quarterly)', percent: 100, color: 'var(--ifm-color-primary)'},
]} />

- **Weekly (~1h)** — Quick review of KPI movements and threshold alerts.
- **Monthly (~3h)** — Structured feedback synthesis and accumulated signals.
- **Quarterly (~5h)** — Strategic session with deep analysis and focus decisions.

</StepAccordionItem>
<StepAccordionItem number={4} title="Step 4. Context Document update" summary="Update the Context Document with full traceability from market signals to context changes.">
...
Maturity level of the continuous learning system.

<RadarChart
  axes={['Signal traceability', 'Context fidelity', 'Validated hypotheses', 'Action on Brief', 'Dashboard health']}
  datasets={[
    {label: 'Operational status', values: [95, 90, 85, 100, 92], color: 'var(--ifm-color-success)'},
    {label: 'Phase target', values: [80, 80, 80, 80, 80], color: 'var(--ifm-color-danger)'},
  ]}
/>

- **Signal traceability** — Every context change has traceability to the market signal that originated it.
- **Context fidelity** — The Context Document faithfully reflects market learnings.
- **Validated hypotheses** — Percentage of Iteration Brief hypotheses verified against real data.
- **Action on Brief** — Every Iteration Brief produces at least one tangible context change.
- **Dashboard health** — KPIs are connected, updated, and with operational alerts.

</StepAccordionItem>
<StepAccordionItem number={5} title="Step 5. The Iteration Brief and the new iteration" summary="Write the Iteration Brief marking the transition between a learning cycle and the next construction cycle.">
