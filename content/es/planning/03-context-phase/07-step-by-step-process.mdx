...
slug: /framework/fases/context-phase/proceso-paso-a-paso
...
sidebar_position: 1
tags: [fases, context-phase, proceso, agentes, reglas, skills]
hide_title: true
---
...
El Context Phase no es un ejercicio de escritura creativa. Es un proceso de traducción estructurado que convierte la comprensión humana capturada en las Fases 1 y 2 en un sistema que la IA puede procesar fielmente. Cada paso tiene un propósito claro, un entregable concreto y un criterio de validación.

<StepAccordion>
<StepAccordionItem number={1} title="Paso 1. Establecer Reglas (project-context.md)" summary="Crear la constitución project-context.md: extraer cada decisión global del Solution Brief y calibrar niveles de precisión.">
...
No todas las Reglas necesitan la misma rigidez. La clave es calibrar: invertir precisión donde el impacto sobre Agentes y Skills es alto, y delegar donde es bajo. El Nivel 5 es restricción absoluta. El Nivel 1 es delegación consciente.

<WeightChart bars={[
  {label: 'Nivel 5 — Prescriptivo', percent: 100, color: 'var(--ifm-color-danger)'},
  {label: 'Nivel 4 — Específico', percent: 80, color: 'var(--ifm-color-warning)'},
  {label: 'Nivel 3 — Dirigido', percent: 60, color: 'var(--ifm-color-primary)'},
  {label: 'Nivel 2 — Orientativo', percent: 40, color: 'var(--ifm-color-info)'},
  {label: 'Nivel 1 — Delegado', percent: 20, color: 'var(--pdai-gray-300)'},
]} />
...
<StepAccordionItem number={2} title="Paso 2. Diseñar la Cadena de Agentes" summary="Definir la cadena de Agentes especializados que traducirá el Solution Brief en un sistema de contexto completo.">
...
La mayoría de proyectos usan cinco Agentes en secuencia. Cada Agente transforma el output del anterior pero nunca lo contradice, nunca lo enriquece con invención y nunca lo reinterpreta. Como una carrera de relevos: cada corredor lleva el mismo testigo.

<EffortChart segments={[
  {label: 'Analyst Agent', percent: 20, color: 'var(--ifm-color-primary)'},
  {label: 'PM Agent', percent: 20, color: 'var(--ifm-color-danger)'},
  {label: 'Architect Agent', percent: 20, color: 'var(--ifm-color-warning)'},
  {label: 'SM Agent', percent: 20, color: 'var(--ifm-color-info)'},
  {label: 'QA Agent', percent: 20, color: 'var(--ifm-color-success)'},
]} />

- **Analyst Agent** — Traduce artefactos humanos en requisitos estructurados. No descubre ni inventa — reformatea.
- **PM Agent** — Genera el PRD: qué construir, para quién, con qué prioridad. Define casos de uso y criterios de éxito.
- **Architect Agent** — Traduce el PRD en decisiones técnicas. El único Agente que actualiza el project-context.md con nuevas Reglas.
- **SM Agent** — Descompone la solución en Skills autocontenidos. Cada Skill incluye todo el contexto para su Agente ejecutor.
- **QA Agent** — Valida fidelidad, completitud, consistencia con Reglas. Verifica en cada eslabón de la cadena, no solo al final.
...
<StepAccordionItem number={3} title="Paso 3. Generar Documentos Estratégicos" summary="Producir el PRD y el Architecture Document — la capa intermedia entre Reglas y Skills.">
...
<StepAccordionItem number={4} title="Paso 4. Descomponer en Skills" summary="Descomponer todo en Skills autocontenidos que los Agentes de desarrollo ejecutarán.">
...
<StepAccordionItem number={5} title="Paso 5. Validar a través de Outputs" summary="Ejecutar el sistema, juzgar los resultados e iterar hasta que los outputs expresen fielmente el Problem Statement.">
